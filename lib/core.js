...
var characterhanlder = require('./characterhandler.js');
...
var characterParsing = function( str ){
  ...
  for (var i = 0; i < codeHtmlList.length; i++) {
    ...
    // 格式化代码块内的代码tab
    var publiTab = null;
    var l = resultStr.split(sourceStr);
    if( l.length > 1 ){
      publiTab = l[0].match(/[\$\/s]+$/g);
    }
    if(publiTab[0]){
      copyStr = copyStr.replace(/[\$\/s]+/g, function(expression){ 
       return expression.replace(publiTab[0],'');
      })
      // while( copyStr.indexOf(publiTab[0]) != -1 ){
      //    copyStr = copyStr.replace(publiTab[0],'');
         
      // }
    }

    // 将代码块内的空格替换成特殊字符以作为保留
    copyStr = copyStr.replace(/\$\/s/g, '&$/s');

    // 去除代码块内结尾多余的空格
    var tempList = copyStr.split('```')
    for (var i = 0; i < tempList.length; i++) {
      if( i > 0 && i%2 == 1){
        tempList[i] = tempList[i].replace(/(\&\$\/s)*$/g,'')+'```'
      }
    }
    copyStr = tempList.join('```');
...
module.exports = function (source) {
  ...
  // var content = characterhanlder(parser, source, params)
